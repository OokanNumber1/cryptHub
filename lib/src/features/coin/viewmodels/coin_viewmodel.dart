...
final gainersLocalVM = FutureProvider<List<CmcToken>>(
    (ref) => LocalRepository().get('gainers'));
final losersLocalVM = FutureProvider<List<CmcToken>>(
    (ref) => LocalRepository().get('losers'));
//*/
...
final allCoinViewmodel = FutureProvider<List<CmcToken>>((ref) async {
  if (await InternetConnectionChecker().hasConnection) {
    //Provider<NetworkAbstract>' can't be assigned to the parameter type 'NetworkAbstract'.
    //return TokenRepository(networkService: networkService).getAllTokens(AppStrings.coinUrl);
///*
